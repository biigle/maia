<div v-if="filterTpTabOpen" v-cloak class="maia-content">
    <tp-image-grid :images="trainingProposals" empty-url="{{ asset(config('thumbnails.empty_url')) }}" :width="{{config('thumbnails.width')}}" :height="{{config('thumbnails.height')}}" :initial-offset="filterTpOffset" v-on:select="handleSelectedTrainingProposal" v-on:deselect="handleDeselectedTrainingProposal" v-on:scroll="updateFilterTpOffset" ref="imageGrid"></tp-image-grid>
